...
# Process WAV audio files - convert to MP3
process_wav() {
    local file="$1"
    local backup_dir="${2:-./backups}"
    ...
    # Check if file has been processed
    if is_file_processed "$file"; then
        echo "Skipping already processed WAV file: $file"
        return 0
    fi
    ...
    if [ "$dry_run" = "true" ]; then
        echo "[DRY RUN] Would convert WAV to MP3: $file"
        return 0
    fi
    ...
    # Create output filename
    local output_file="${file%.*}.mp3"

    # Use ffmpeg to convert WAV to MP3
    if ffmpeg -i "$file" -vn -ar 44100 -ac 2 -b:a 192k -map_metadata -1 "$output_file"; then
        # Remove original file if conversion successful and not backing up
        if [ "$backups" = "false" ]; then
            rm "$file"
        fi
        echo "✓ Converted WAV to MP3: $output_file"
        # Log processed file
        log_processed_file "$output_file"
        return 0
    else
        echo "✗ Failed to convert WAV to MP3: $file"
        return 1
    fi
}

# Process OGG audio files - convert to MP3
process_ogg() {
    local file="$1"
    local backup_dir="${2:-./backups}"
    ...
    # Check if file has been processed
    if is_file_processed "$file"; then
        echo "Skipping already processed OGG file: $file"
        return 0
    fi
    ...
    if [ "$dry_run" = "true" ]; then
        echo "[DRY RUN] Would convert OGG to MP3: $file"
        return 0
    fi
    ...
    if ffmpeg -i "$file" -vn -ar 44100 -ac 2 -b:a 192k -map_metadata -1 "$output_file"; then
        ...
        echo "✓ Converted OGG to MP3: $output_file"
        # Log processed file
        log_processed_file "$output_file"
        return 0
    else
        echo "✗ Failed to convert OGG to MP3: $file"
        return 1
    fi
}

# Process IFF audio files - convert to MP3
process_iff() {
    local file="$1"
    local backup_dir="${2:-./backups}"
    ...
    # Check if file has been processed
    if is_file_processed "$file"; then
        echo "Skipping already processed IFF file: $file"
        return 0
    fi
    ...
    echo "Processing IFF: $file"
    if [ "$dry_run" = "true" ]; then
        echo "[DRY RUN] Would convert IFF to MP3: $file"
        return 0
    fi
    ...
    # Use ffmpeg to convert IFF to MP3
    if ffmpeg -i "$file" -vn -ar 44100 -ac 2 -b:a 192k -map_metadata -1 "$output_file"; then
        # Remove original file if conversion successful and not backing up
        if [ "$backups" = "false" ]; then
            rm "$file"
        fi
        echo "✓ Converted IFF to MP3: $output_file"
        # Log processed file
        log_processed_file "$output_file"
        return 0
    else
        echo "✗ Failed to convert IFF to MP3: $file"
        return 1
    fi
}
...
process_files() {
    ...
    while IFS= read -r file; do
        ...
        case "$ext" in
            ...
            wav)
                # Process WAV audio files - convert to MP3
                process_wav "$file" "$backup_dir"
                ;;
            ogg)
                # Process OGG audio files - convert to MP3
                process_ogg "$file" "$backup_dir"
                ;;
            iff)
                # Process IFF audio files - convert to MP3
                process_iff "$file" "$backup_dir"
                ;;
            m4v|mkv|mp4)
